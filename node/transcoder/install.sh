...
# install ufw
DEBIAN_FRONTEND=noninteractive apt install -yy ufw
...
mkdir -p /mnt/streaming
mkdir -p /mnt/streaming/hls
mkdir -p /mnt/streaming/keys
mkdir -p /mnt/streaming/dash
mkdir -p /mnt/vod
mkdir -p /var/www/html
mkdir -p /opt/zeus

# set ownership
chown -R www-data:www-data /mnt/recordings /mnt/streaming /var/www/html /opt/zeus

# mount nfs share for vod
# mount -t nfs -o mountvers=3 10.254.205.21:/mnt/vol /mnt/vod

# enter nginx folder
cd /etc/nginx

# replace nginx.conf
wget --no-check-certificate -O /etc/nginx/nginx.conf http://git.genexnetworks.net/whittinghamj/pdx-project-zeus-v2/-/raw/main/node/transcoder/nginx.conf

# download self signed ssl
mkdir -p /etc/nginx/ssl
wget --no-check-certificate -O /etc/nginx/ssl/server.crt http://git.genexnetworks.net/whittinghamj/pdx-streamer-live/-/raw/main/ssl/server.crt
wget --no-check-certificate -O /etc/nginx/ssl/server.csr http://git.genexnetworks.net/whittinghamj/pdx-streamer-live/-/raw/main/ssl/server.csr
wget --no-check-certificate -O /etc/nginx/ssl/server.key http://git.genexnetworks.net/whittinghamj/pdx-streamer-live/-/raw/main/ssl/server.key
...
# install ramfs drives for streams and nfs share
cp /etc/fstab /etc/fstab.bak
echo "tmpfs /mnt/streaming tmpfs defaults,noatime,nosuid,nodev,noexec,mode=1777,size=90% 0 0" >> /etc/fstab
echo "10.254.205.21:/mnt/vol /mnt/vod nfs rw,soft,intr,rsize=8192,wsize=8192,timeo=14 0 0" >> /etc/fstab

mount -t tmpfs -o size=90% streaming /mnt/streaming
...
# download php files
wget --no-check-certificate -O /opt/zeus/nginx_active_streams.php http://git.genexnetworks.net/whittinghamj/pdx-project-zeus-v2/-/raw/main/cms/agent/nginx_active_streams.php
wget --no-check-certificate -O /var/www/html/redirect.php http://git.genexnetworks.net/whittinghamj/pdx-project-zeus-v2/-/raw/main/node/transcoder/redirect.php

## save server_guid
echo $SERVER_GUID > /opt/zeus/server_guid

## install crontab
wget --no-check-certificate -O /opt/zeus/crontab.txt http://git.genexnetworks.net/whittinghamj/pdx-project-zeus-v2/-/raw/main/node/transcoder/crontab.txt
crontab /opt/zeus/crontab.txt

## get release_version.txt
wget --no-check-certificate -O /opt/zeus/release_version.txt http://git.genexnetworks.net/whittinghamj/pdx-project-zeus-v2/-/raw/main/node/transcoder/release_version.txt
